...
const mealLabels: Record<string, MealLabel> = {
  breakfast: { icon: "🌅", label: "朝ごはん", color: "text-[#D9768A]", borderColor: "border-[#F9C6D0]", bgColor: "bg-[#FFF5F7]" },
  lunch: { icon: "☀️", label: "昼ごはん", color: "text-[#7a6010]", borderColor: "border-[#FAE095]", bgColor: "bg-[#FFFDF0]" },
  dinner: { icon: "🌙", label: "夜ごはん", color: "text-[#2a6649]", borderColor: "border-[#C5E8D8]", bgColor: "bg-[#F5FBF7]" },
};

const toDateKey = (isoString: string) => isoString.split("T")[0];

export default function History() {
  // --- カスタムフックの呼び出し（Reactのルールに従いコンポーネント内に移動！） ---
  const {
    startDate,
    endDate,
    setStartDate,
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setFirestoreMeals(
          mealsSnap.docs.map((d) => ({ id: d.id, ...d.data() }) as FirestoreMeal)
        );
        setFirestoreDiary(
          diarySnap.docs.map((d) => ({ id: d.id, ...d.data() }) as FirestoreDiary)
        );
      } catch (e) {
        console.error(e);
      } finally {
  ...
  const threeMealDays = allDates.filter((d) => mealsByDate[d].length >= 3).length;
    ...
    <div className="min-h-screen bg-[#FDF6F0] p-4 text-[#4a3f3a] font-sans antialiased pb-10">
      <div className="max-w-md mx-auto space-y-4">
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-7 gap-1 text-center">
                {["月", "火", "水", "木", "金", "土", "日"].map((d) => (
                  <span key={d} className="text-[11px] text-gray-400 font-bold mb-1">{d}</span>
                ))}
                ...
                {[...Array(30)].map((_, i) => {
                  ...
                  return (
                    <button
                      key={day}
                      onClick={() => meals && setSelectedDate(isSelected ? null : dateKey)}
                      disabled={!meals}
                      className={`aspect-square flex flex-col items-center justify-center rounded-xl transition ${bgColor} ${
                        meals ? "cursor-pointer active:scale-95" : "cursor-default opacity-40"
                      }`}
                    >
                      <span className={`text-[10px] font-bold ${
                        isSelected ? "text-white" : isToday ? "text-[#c0506a]" : "text-gray-700"
                      }`}>
                        {day}
                      </span>
                      {meals && (
                        <span className={`text-[7px] font-semibold mt-0.5 ${isSelected ? "text-white/90" : "text-gray-400"}`}>
                          {meals.length}/3
                        </span>
                      )}
                    </button>
              ...
              <p className="text-center text-[10px] text-gray-400 pt-1">📌 日付をタップするとその日の食事が見れるよ</p>
            </>
          )}
        </div>
        ...
        {selectedDate && (
          <div className="bg-white rounded-2xl p-4 border-[1.5px] border-[#F9C6D0] space-y-3 shadow-sm animate-fade-in">
            ...
            {selectedMeals.length === 0 ? (
              <p className="text-center text-xs text-gray-400 py-2">この日の記録はないよ🌱</p>
            ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 space-y-0.5">
                          <p className="text-xs font-bold text-[#4a3f3a]">{config?.label || meal.mealType}</p>
                          <p className="text-[10px] text-gray-400">🕐 {formatTime(meal.date)}</p>
                          {meal.note && <p className="text-xs text-[#6a5060] leading-relaxed mt-1">{meal.note}</p>}
                        </div>
                      </div>
                    );
                  })}
            ...
            {selectedMeals.length > 0 && selectedMeals.length < 3 && (
              <p className="text-center text-[11px] text-gray-400 bg-gray-50 p-2 rounded-xl leading-relaxed">
                {3 - selectedMeals.length}食分は記録されていないよ。次はもう少し食べられるといいね🌱
              </p>
            )}

            {selectedDiary && (
              <div className="bg-[#FFF5F7] rounded-xl p-3 border border-[#F9C6D0] space-y-1">
                <p className="text-[11px] font-bold text-[#D9768A]">📝 その日の日記</p>
                <p className="text-xs text-[#6a5060] leading-relaxed">{selectedDiary.note}</p>
              </div>
            )}
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl p-4 border-[1.5px] border-[#F9C6D0] space-y-3 shadow-sm">
          <h2 className="text-sm font-extrabold text-[#4a3f3a]">今月のまとめ</h2>
          <div className="flex justify-around py-2">
            <div className="text-center">
              <p className="text-2xl font-extrabold text-[#D9768A]">{totalDays}</p>
              <p className="text-[10px] text-gray-400 mt-0.5">記録した日</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-extrabold text-[#D9768A]">{threeMealDays}</p>
              <p className="text-[10px] text-gray-400 mt-0.5">3食達成🎉</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-extrabold text-[#D9768A]">{totalMeals}</p>
        ...
        <div className="bg-white rounded-2xl p-4 border-[1.5px] border-[#C5E8D8] space-y-3 shadow-sm">
          <h2 className="text-sm font-extrabold text-[#2a6649]">📋 診察用レポート</h2>
          ...
          <div className="flex gap-2 justify-center">
            <button onClick={setThisWeek} className="bg-[#F0FFF8] border border-[#C5E8D8] rounded-full px-4 py-1.5 text-xs font-bold text-[#2a6649] hover:bg-[#e2f7ed] transition">
              今週
            </button>
            <button onClick={setThisMonth} className="bg-[#F0FFF8] border border-[#C5E8D8] rounded-full px-4 py-1.5 text-xs font-bold text-[#2a6649] hover:bg-[#e2f7ed] transition">
              今月
            </button>
            <button onClick={setLastMonth} className="bg-[#F0FFF8] border border-[#C5E8D8] rounded-full px-4 py-1.5 text-xs font-bold text-[#2a6649] hover:bg-[#e2f7ed] transition">
              先月
            </button>
          </div>
          ...
          {/* 出力ボタン */}
          <button
            onClick={() => generatePDF(mealsByDate, diaryByDate, startDate, endDate)}
